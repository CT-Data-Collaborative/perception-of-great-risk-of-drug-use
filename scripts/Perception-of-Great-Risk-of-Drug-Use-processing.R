...
##################################################################
#
# Processing Script for Perception of Great Risk of Drug Use
...
for (i in 1:length(legacy_table_dfs)) {
  ...
  current_table <- current_table[grepl("Perception", current_table$`Table Description`),]
...
new_table_dfs <- new_table_dfs[grepl(paste(c(" 4", " 11", " 14"), collapse = "|"), new_table_dfs)]
...
test$`Perception of Great Risk of Drug Use` <- NA
test$`Perception of Great Risk of Drug Use`[grep("Marijuana", test$`Table Description`)] <- "Perceptions of Great Risk of Smoking Marijuana Once a Month"
test$`Perception of Great Risk of Drug Use`[grep("Alcoholic", test$`Table Description`)] <- "Perceptions of Great Risk of Having Five or More Drinks of an Alcoholic Beverage Once or Twice a Week"
test$`Perception of Great Risk of Drug Use`[grep("Cigarettes", test$`Table Description`)] <- "Perceptions of Great Risk of Smoking One or More Packs of Cigarettes per Day"
...
#assign variable
all_data_long$Variable[grep("Estimate", all_data_long$Variable)] <- "Perception of Great Risk of Drug Use"
all_data_long$Variable[grep("MOE", all_data_long$Variable)] <- "Margins of Error"

#assign measure type
all_data_long$`Measure Type` <- "Percent"

all_data_long$Value <- round(as.numeric(all_data_long$Value), 2)

#Select and sort columns
all_data_final <- all_data_long %>% 
  select(`Substate Region`, Year, `Age Range`, `Perception of Great Risk of Drug Use`, `Measure Type`, Variable, Value) %>% 
  rename(Region = `Substate Region`) %>% 
  arrange(Region, Year, `Age Range`, `Perception of Great Risk of Drug Use`, Variable)

# Write to File
write.table(
  all_data_final,
  file.path(getwd(), "data", "perception-of-great-risk-of-drug-use-2014.csv"),
  sep = ",",
  na = "-9999",
  row.names = F
)
